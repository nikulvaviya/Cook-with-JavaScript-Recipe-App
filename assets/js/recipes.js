...
const /* {NodeElement} */ $filterTogglers = document.querySelectorAll(
    "[data-filter-toggler]"
  );
...
const renderRecipe = (data) => {
  data.hits.map((item, index) => {
    const { image, label: title, totalTime: cookingTime, uri } = item.recipe;

    if (!uri) {
      console.error(`Missing uri for item at index ${index}`, item);
      return;
    }
